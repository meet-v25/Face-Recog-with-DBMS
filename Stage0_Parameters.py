...
dir_name       = 'Swim_Pool_DB_Folder'      # Folder name in which we want to store everyone's raw data
temp_dir_name  = '@last_temp'               # Folder in which raw data of current scan gets tempraritly stored
temp_dir_path  = os_path.join(dir_name, temp_dir_name)
file_name      = 'Swim_Pool_DB_File.xlsx'   # Microsoft Excel file in which whole "Data" is stored
sheet_name     = 'Swim_Pool_Database'       # Excel Sheet's name in the above file

size = 4                # Dividing Factor / Save-Image Size reduction factor
...
''' From the following 3 lines, keep 1 line uncommented, whose HAAR-classifier you want to use '''
